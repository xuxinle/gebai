...
│   ├── my_agent/
│   │   ├── my_agent.ts        # export const def: SubAgentDef = {...}
│   │   └── my_agent.md        # （可选）系统提示词
│   └── another/               # 纯 md 亦可：another/another.md 单独存在即子代理
...
- typecheck：根目录 `bun run typecheck:custom`

## 迁移

新版本歌白发布后：`cp -r custom/ <新仓库根>/`，重启即生效。二开资产与上游升级互不干扰。
