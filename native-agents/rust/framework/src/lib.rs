// Rust 语言基础框架（歌白多语言子代理协议 v2，见 native-agents/README.md）。
//
// 单文件零依赖（纯标准库）：语言目录 native-agents/rust/ 下共享本 crate；每个子代理项目
// （语言目录下的二级目录）一个 main.rs——依赖 gebai-native-framework 后用 framework::register_tool
