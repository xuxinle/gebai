...
import { saveMiniTool, deleteMiniTool, listMiniTools, getMiniTool } from "../../core/widgets-store/mini-tools"
...
export const description =
  "管理 HTML 小工具（Web 标题栏「小工具」面板中加载的可复用页面组件，与模型工具是两回事）：保存/列出/读取/删除。需要创建或维护小工具时装载本子Agent；制作流程通常先用全局 show 工具（html 内容源）在聊天内调试页面，满意后保存。输入：小工具需求或维护指令；输出：保存/清单/源码/删除结果。"
export const systemPrompt =
  "你是 HTML 小工具管理助手。小工具 = 保存在服务端、经 Web 标题栏「小工具」面板随时加载的 HTML 页面组件（沙箱 iframe 渲染，脚本可执行但隔离于宿主页面），**与模型工具（tool call）是两回事**。工作流程：\n" +
  "1) 创建：先在聊天内用全局 show 工具传 html 调试页面（布局/交互/数据确认满意），再 save 保存——name 仅限字母/数字/下划线/中文（1-40 字符，不含 . / 等分隔符），scope=private 用户私有（默认）/ public 公用（所有用户可见），同名覆盖；\n" +
  "2) 查看：list 列出全部小工具（名称/范围/更新时间），get 按名读取完整源码（私有同名优先于公用）；\n" +
  "3) 修改：get 读取现有源码 → 修改 → save 同名覆盖 → 建议再用 show 复核效果后再收尾；\n" +
...
const save: Tool = {
  ...
  description: "保存 HTML 小工具（同名覆盖）。建议先用 show 工具（html 内容源）调试满意后再保存。scope 参数选择可见范围（private 用户私有默认 / public 公用）。",
...
const list: Tool = {
  ...
  async execute(_args, ctx) {
    ...
    if (!items.length) return { output: "暂无小工具。可先在聊天内用 show 工具传 html 调试页面，满意后用 save 保存。", data: { items: [] } }
