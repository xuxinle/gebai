...
export const writeGuard = (env: Record<string, string>, absPaths: string[]): string | null => {
  ...
  for (const target of absPaths) {
    ...
    if (!writable) {
      return (
        `拒绝写入 ${target}：self_optimize 默认只读模式仅允许修改 子Agent 包（packages/agents/src/）与仓库级文档/配置（DESIGN.md/AGENTS.md/.env.example/README.md/kilo.json），` +
