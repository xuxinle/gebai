 ...
 *  不可中断**（连 setTimeout 回调也要排在它后面），期间该进程全部会话冻结。
 *  本项目仅部署 Bun（JSC），其回溯预算只约束**单次**调用（实测上界随形态而异：`(a+)+b` ~0.78s、
 *  `(a*)*b` ~2.1s、`(a|a)+b` 长输入 3.5s），而本类工作是「每文件 × 每行」各调一次——
 *  实测 20 次调用即 >10s；且该预算属引擎内部实现（非文档化契约）。
 *  故隔离到子进程 + 超时强杀（详见 fs.ts 的 GREP_MATCHER_TIMEOUT_MS 注释）。 */
