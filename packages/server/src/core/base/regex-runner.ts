 ...
 *  模型提供的正则可能含嵌套量词等灾难性回溯形态（如 (a+)+b 配超长单行）——正则 test 是**同步 CPU 操作、
 *  不可中断**，一旦进入就无法中断（连超时回调也要排队），期间该进程全部会话冻结。实测 `(a+)+b` 
 *  失配输入：JSC（Bun）有回溯预算缓解（单次有界 ~0.77s，且结果正确不误判），V8（Node）无缓解
 *  （n=30 即指数挂死）——**不能依赖运行时的缓解**（随部署运行时不同而不同，且缓解只保证单次有界、
 *  不保证「文件×行」整体有界），故隔离到子进程 + 超时强杀。 */
import { spawn } from "node:child_process"
import { tmpdir } from "node:os"
import { join } from "node:path"
import { jsRuntimeCommand } from "../exec/js-tool"

/** 匹配超时：超时强杀子进程并返回引导（不回退进程内匹配——回退即重新暴露挂死面：同步不可中断）。 */
