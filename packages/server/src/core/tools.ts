...
import { normalizeBranchSpecs, type BranchRunRecord } from "./branch-runs"
...
export function createAllGlobalTools(): Record<string, Tool> {
  ...
  return {
    ...
    // branch_run（会话分支运行与合并，DESIGN「会话分支运行与合并」）：主上下文 fork 多分支并行执行+自动合入，
    // git 式并发——多路模型接口（GEBAI_LLM_ROUTES）摆脱单轮串行速度限制；引擎仅主循环注入 branchRuns 服务
    branch_run: branchRunTool,
