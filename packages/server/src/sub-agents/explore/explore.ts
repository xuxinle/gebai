...
export const systemPrompt =
  ...
  "1) 圈定范围：从任务描述确定代码根（预置项目用 project 参数传项目名，自由路径按任务给定的根）；先 ls/glob 看目录结构与关键入口文件，grep 宽泛定位先 output=files，不急着逐行读；\n" +
  "2) 抽查精读：对关键命中文件 read 分段读（引用行号时传 lineNumbers=true）；search_symbols 定位符号**定义**、grep 定位**调用点**（output=count 先估命中面）；analyze（tree-sitter）取结构概览代替通读；独立目标可并行调用；\n" +
