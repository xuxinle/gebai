...
export const systemPrompt =
  ...
  "3) 通知 notify（强烈建议为无人值守任务配置）：通道数组，每条 {type,target,webhookId,secret,at}——type=webhook（任意 http(s) 回调 POST JSON，可直配 target URL 或以 webhookId 引用 REST /api/v1/webhooks 已注册的事件 Webhook——投递自动带注册密钥的 X-Gebai-Signature HMAC 签名，注册侧改动即时生效）、feishu（飞书群自定义机器人 webhook 地址，secret 为加签密钥可选）、feishu_chat（飞书应用消息，target 填群 chat_id，需服务端配置飞书应用凭证）；飞书通知默认以 **markdown 卡片**发送（状态着色头部 + 粗体字段 + 输出摘要），可配 at 名单 @ 人（open_id 或 \"all\" @所有人——注意：at 含 \"all\" 时自动降级为文本消息发送，飞书卡片不支持 @所有人）；webhook 通道的 at 名单随 JSON 载荷 at 字段携带（接收方系统据此渲染 @ 人），配 secret 时载荷带 HMAC 签名；notifyOn=always（缺省每次通知）/error（仅失败）；运行结果/超时/自动停用会推送到全部通道，通知失败不影响任务执行；\n" +
...
function notifyParam(): Record<string, unknown> {
  return {
    type: "array",
    description:
      "通知通道数组（可选）：每条 {type,target,webhookId,secret,at}——webhook（http(s) 回调 URL，或 webhookId 引用 REST /api/v1/webhooks 已注册的事件 Webhook——投递自动带其 HMAC 签名，二选一）/ feishu（飞书群自定义机器人 webhook 地址）/ feishu_chat（飞书 chat_id，需服务端飞书应用凭证）；secret 为密钥（feishu 加签 / webhook 直配时 HMAC 签名）；at 为 @ 人名单（飞书渲染进卡片；webhook 随 JSON 载荷 at 字段携带供接收方解析），通知以 markdown 卡片发送（webhook 为 JSON）",
    items: {
      type: "object",
      properties: {
        type: { enum: ["webhook", "feishu", "feishu_chat"], description: "通道类型" },
        target: { type: "string", description: "webhook/feishu=URL；feishu_chat=群 chat_id；webhook 用 webhookId 引用时可省略" },
        webhookId: { type: "string", description: "引用 REST /api/v1/webhooks 注册的事件 Webhook id（仅 webhook 通道，与 target 二选一，投递带注册密钥的 X-Gebai-Signature 签名）" },
...
function parseNotify(raw: unknown): CronNotifyChannel[] | undefined {
  ...
  const out = raw.map((ch) => {
    ...
    return {
      ...
      webhookId: c.webhookId != null ? String(c.webhookId) : undefined,
  ...
  parameters: schema(
    {
      ...
      sessionId: { type: "string", description: "target=session 时可指定绑定会话 id（缺省为当前会话）" },
      ...
      timeoutMs: { type: "number", description: "单次执行超时毫秒（可选，缺省脚本 300000 / 提示词 1800000）" },
      maxConsecutiveErrors: { type: "number", description: "连续失败自动停用阈值（可选，缺省 0 不停用；建议无人值守任务配 3~10）" },
      notifyOn: { enum: ["always", "error"], description: "通知时机（可选，缺省 always；error=仅失败时通知）" },
      notify: notifyParam(),
    ...
    const task = await ctx.cron.add(
      {
        ...
        maxConsecutiveErrors: args.maxConsecutiveErrors != null ? Number(args.maxConsecutiveErrors) : undefined,
        notifyOn: args.notifyOn != null ? (String(args.notifyOn) as "always" | "error") : undefined,
  ...
  parameters: schema(
    {
      ...
      sessionId: { type: "string", description: "target=session 绑定的会话 id" },
      ...
      timeoutMs: { type: "number", description: "单次执行超时毫秒" },
      maxConsecutiveErrors: { type: "number", description: "连续失败自动停用阈值（0=不停用）" },
      notifyOn: { enum: ["always", "error"], description: "通知时机" },
      notify: notifyParam(),
    },
    ["id"],
  ),
