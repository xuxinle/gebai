...
export interface TutorStageConfig {
  /** 学科清单（subject 参数 description）。 */
  subjects: string
  /** 年级取值说明（grade 参数 description）。 */
  grades: string
  /** 命名空间前缀（输出文案引用工具全名，如 tutor_secondary）。 */
  prefix: string
}

export const SECONDARY_STAGE: TutorStageConfig = {
  subjects: "语文/数学/英语/物理/化学/生物/政治/历史/地理",
  grades: "初一/初二/初三/高一/高二/高三",
  prefix: "tutor_secondary",
}

export const PRIMARY_STAGE: TutorStageConfig = {
  subjects: "语文/数学/英语/科学/道德与法治（低年级可为 拼音/识字/口算 等具体领域）",
  grades: "小学一年级~六年级（如 三年级）",
  prefix: "tutor_primary",
}
...
export function createTutorTools(cfg: TutorStageConfig): { tools: Record<string, Tool>; requiresApproval: Record<string, boolean> } {
  const profile: Tool = {
    name: "profile",
    description:
    ...
    parameters: schema({
      ...
      exams: { type: "string", description: "考试日程（如 6月17-19 期末考）" },
      notes: { type: "string", description: "教学偏好与其他备注（如 只要思路不要答案、需要家长陪读）" },
    }),
  ...
  const knowledge: Tool = {
    ...
    parameters: schema({
      subject: { type: "string", description: `学科（设置必填；查询时可单独传作过滤。${cfg.subjects}）` },
      topic: { type: "string", description: "知识点（如 一元二次方程、拼音声调；设置必填，查询留空列整科）" },
  ...
  const mistakeAdd: Tool = {
    ...
    parameters: schema(
      {
        subject: { type: "string", description: `学科（${cfg.subjects}）` },
        topic: { type: "string", description: "知识点（如 一元二次方程、进位加法）" },
        ...
        analysis: { type: "string", description: "错因分析与正确思路（简明，注明错因类型：概念混淆/方法不会/计算失误/审题失误/抄错题）" },
        source: { type: "string", description: "题目来源（如 作业、单元测试卷）" },
      },
    ...
    async execute(args, ctx): Promise<ToolResult> {
      ...
      return {
        output: `错题已登记: ${m.id}（${m.subject}·${m.topic}）\n下次复习: ${fmtDate(m.nextReviewAt)}。复习时用 ${cfg.prefix}_mistake_list 取题、学生重做后用 ${cfg.prefix}_mistake_review 汇报结果。`,
  ...
  const mistakeReview: Tool = {
    name: "mistake_review",
    description:
      "错题复习结果汇报：学生重做该题后按结果汇报，自动排下次复习。pass=这次做对（间隔递增 1/3/7/14/30 天，连续 5 次通过自动掌握）；fail=又错（间隔重置为 1 天）；master=已彻底掌握（直接归档）。复习流程：先取到期清单让学生重做，再汇报。",
    card: { titleParams: ["id", "result"] },
    parameters: schema(
      {
        id: { type: "string", description: "错题 ID（清单查看）" },
    ...
    async execute(args, ctx): Promise<ToolResult> {
      ...
      if (!m) return { output: `错题不存在或 id 非法: ${args.id}（可用 ${cfg.prefix}_mistake_list 查看）。` }
  ...
  const mistakeRemove: Tool = {
    name: "mistake_remove",
    description: "删除错题（按 id，不可恢复，需审批）。仅学生明确要求移除时使用；只是「做对了」不移除——用 mistake_review 走掌握流程。",
    ...
    async execute(args, ctx): Promise<ToolResult> {
      ...
      return removed
        ...
        : { output: `错题不存在或 id 非法: ${args.id}（可用 ${cfg.prefix}_mistake_list 查看）。` }
    },
  }
