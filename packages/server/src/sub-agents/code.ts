...
export const systemPrompt =
  ...
  "2) 探索：先用 grep（内容搜索）/search_files（按文件名 glob 查找）/search_symbols（按符号名定位**定义**位置，跨文件；找**引用/调用点**用 grep）/ls（目录结构）/analyze（tree-sitter 结构概览）快速定位，再精确读取相关文件（大文件用 read 的 offset/limit 分段读，避免整读超长输出），避免大范围逐行通读；对独立的目标可一次发起多个并行工具调用；\n" +
  ...
  "7) 收尾：用 git 工具只读查看变更（status/diff/log，无需审批）确认改动范围，只提交预期文件，不擅自 commit（add/commit 等写操作用 sh 且需审批；与本次任务无关的既有改动不要误动）；用 todo（空 entries 查询）核对全部待办后给出总结。\n" +
 ...
 */
function projectAware(tool: Tool, opts: { workdir?: boolean } = {}): Tool {
