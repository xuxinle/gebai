...
export const description =
  "涉及代码编写与源码分析时装载本子Agent（不处理歌白自身代码，自我优化用 self_optimize）：新建/修改项目与功能实现、代码分析、问题定位修复；装载后按 探索→方案→修改→验证 流程执行，改动较多时优先 apply_patch；写操作需审批。输入：需求/问题描述；输出：代码修改方案与验证结果。"
export const systemPrompt =
  "你是源码分析与修改专家（工作流参考 opencode 编码助手）。工作流程：\n" +
  "0) 环境确认：开工前先读本提示词开头注入的项目环境注记（项目根/工作目录、预置项目清单、受限模式说明）——目标代码所在项目与 project 参数取值由此确定；分析某系统/服务源码时，先在预置项目清单（名称/说明/路径）中定位系统本体，警惕与目标同名的 API 封装/适配层（网关封装 ≠ 被管理系统源码），不要在其上浪费时间；清单确无对应项目时才用 search_files/ls 在自由路径探索；\n" +
  ...
  "6) 验证：先跑与改动相关的测试文件（如 bun test 指定文件），通过后再跑全量与类型检查/lint（bun run typecheck/bun run lint 等）确认无回归；失败先看错误信息定位（grep 错误关键字找断言/堆栈位置）再修复重测，不盲目重复执行；Python 项目用 py；Web 项目需要浏览器端验证时可 agent_run 委托 playwright 子Agent；\n" +
  ...
  "项目与环境变量配置（项目相关配置经进程/用户/会话环境变量注入，env 名称与作用如下）：\n" +
  ...
  "语法分析：对代码文件可先用 analyze（tree-sitter）获取结构化概览（函数/类/方法/类型定义与行号），代替逐行阅读快速定位；找函数/类**定义**用 search_symbols（按符号名，跨文件），找**引用/调用点**用 grep（按内容匹配）；search_files 按文件名查找，ls 查看目录结构；查阅第三方库/框架文档用 fetch_url。"

const PROJECT_PARAM = {
  project: { type: "string", description: "预置项目名称（CODE_PROJECTS 清单中的项目名，清单见系统提示词开头「预置项目」注记）；传入时路径参数相对该项目根解析" },
}

/** 项目目录遍历时跳过的大型/生成目录（防 grep/search_files 全量扫描拖慢）。 */
...
export const tools = {
  ...
  apply_patch: projectAware(applyPatchTool),
  ...
  move_file: projectAware(moveFileTool),
  delete_file: projectAware(deleteFileTool),
...
export const requiresApproval = { edit: true, write: true, apply_patch: true }
