 ...
 * REST 确认 docqa 子代理注册 → WS sendPrompt 驱动模型按场景脚本调用
 * docqa_index + docqa_query（索引→检索全链路）→ 断言工具输出。
 *
...
// 1) REST 子代理目录：docqa 注册面
const subs = await c.get("/api/v1/sub-agents")
const names = ((subs ?? []) as Array<{ name: string }>).map((a) => a.name)
console.log("REST /sub-agents:", names.length, "个，docqa 注册:", names.includes("docqa"))
if (!names.includes("docqa")) throw new Error("docqa 未注册（boot native 接线失败）")

// 2) 新会话任务：fake-llm python 场景（docqa_index → docqa_query → 收尾）
...
for await (const ch of c.sendPrompt(sessionId, "为本地语料建索引并检索边车协议", { env: { GEBAI_APPROVAL_SKIP: "true" } })) {
  if (ch.kind === "tool_result") {
    const tname = (ch as { toolCall?: { name?: string } }).toolCall?.name ?? "?"
    toolCalls.push(tname)
    if (tname === "docqa_index" || tname === "docqa_query") outputs.push(String((ch as { output?: string }).output ?? ""))
...
// 3) 断言：docqa_index 建索引成功（含文档数）、docqa_query 命中片段；
// 首调可能带 agent_load（场景脚本与路由自愈装载兼容两种路径）
const indexCount = toolCalls.filter((n) => n === "docqa_index").length
const queryCount = toolCalls.filter((n) => n === "docqa_query").length
console.log("docqa 输出:", outputs.map((o) => o.slice(0, 60)))
if (!toolCalls.includes("agent_load") && toolCalls[0] !== "docqa_index") throw new Error(`工具序列异常: ${toolCalls}`)
if (indexCount !== 1 || queryCount !== 1) throw new Error(`docqa_index/query 各应调 1 次，实际 ${indexCount}/${queryCount}`)
if (!outputs[0]?.includes("索引完成")) throw new Error(`docqa_index 未成功: ${outputs[0]?.slice(0, 100)}`)
if (!outputs[1]?.includes("命中")) throw new Error(`docqa_query 未返回命中: ${outputs[1]?.slice(0, 100)}`)

// 4) 会话记录落盘核验（chat.json 含工具调用）
const msgs = await c.get(`/api/v1/sessions/${sessionId}`)
console.log("会话记录可见:", !!msgs)

console.log("\n=== 服务级引擎全链路验证通过（boot 接线→路由自愈装载→docqa 索引/检索→输出回传）===")
process.exit(0)
