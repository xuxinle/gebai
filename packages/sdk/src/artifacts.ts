/**
 * 工具产物展示辅助（自 server core/support/artifacts.ts 下沉，sdk 单一来源）：
 * 产物内容块构造 / MIME 与图表格式推断 / PlantUML 规范化 / 预览逻辑路径。
 * server 与 agents 双侧共用（show 工具与子代理工具的产物块同构）。
 */
import { isAbsolute, join, relative, sep } from "node:path"
import type { ContentBlock, DiagramFormat } from "./types"
import type { ToolContext } from "./agent-contract"
import { sessionPath as sessionPathFor } from "./agent-utils"
...
export function previewLogicalPath(absPath: string, ctx: ToolContext): string {
  ...
  try {
    sessionTmp = join(sessionPathFor(ctx.home, ctx.user, ctx.sessionId), "tmp")
  } catch {
