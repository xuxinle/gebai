...
- 模型资产复用歌白 CV 约定（均落在资源子仓库 {GEBAI_HOME}/models/）：GEBAI_CV_MODELS_DIR（det.onnx/rec.onnx/dict.txt 的 PP-OCR 三件套，缺省目录 {GEBAI_HOME}/models/ocr/）与 GEBAI_CV_DETECT_MODEL（或 {GEBAI_HOME}/models/detect/ 唯一 .onnx）。
