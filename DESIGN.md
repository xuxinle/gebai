...
- **裁剪构建样例**（根 `scripts/` 目录，`bun run build:code`）：`build-code-agent.ts` 产出 code 场景精简服务端单文件二进制（`packages/server/dist/gebai-code[.exe]`，浏览器形态、内嵌 Web UI）——三层裁剪组合示范：子Agent 包含清单（code+explore，体积收益主来源：未选子Agent 模块整体摇出产物）+ 预加载清单（code 开箱即用）+ 全局工具排除清单（show/fetch_url）。作为其他场景裁剪构建的模板：复制脚本改三处清单即可（如 reverse_site 站点逆向、feishu 文档、只读分析）
- 命名规则：仅限小写字母、数字、下划线
