...
- **两种打开方式**：标题栏入口主按钮 = **新标签打开**（工作台是重量级独立页面，与聊天并行是常态，且故障隔离）；按钮下缘 hover 弹出的副按钮 = **分屏模式**（右侧 iframe 嵌工作台，左侧主视图折叠会话列表，默认五五开、分界可拖双击复位）。消息流里的文件产物（文件卡 / 链接 / 原文件弹窗）也走同一入口，自动带上所属会话。
