...
- **参数**：`pattern`（正则）+ 可选 `path`（搜索起点：目录（递归）或单个文件（直接内搜该文件，grep 传文件语义）；默认 `.`，显式传 `.` 与省略等价（经 `resolvePath` 归一化，与 `glob` 同一惯例），相对会话工作目录、`tmp/` 前缀可省略）/`ignoreCase`/`output`/`context`/`include`；搜索范围为会话 `tmp/` 子树（与文件面板一致），返回路径带 `tmp/` 前缀（可直接用于 `read` 等文件工具）
- **三种结果形态（`output`）**：
