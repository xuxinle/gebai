...
- **界面结构（无标题栏三区）**：整体对齐 IDEA **新 UI**——去掉传统标题栏/菜单栏，入口交给**左侧活动栏**与各面板自身：
  - 活动栏分两组（中间以细分隔）：**上组 = 视图切换**（资源管理器 / 搜索 / 变更，三视图**互斥**——同一时刻只看一件事：要么在找文件、要么在看改动）；**下组 = 工具窗与全局入口**（Git 工具窗开关 / 回收站 / **更多**）；**嵌入（分屏）时最下方多一个「关闭分屏」**（图标与「更多」里那一项同款 `back`，走既有的 `requestCloseSplit()`）——面板内最直接的那个出口，独立标签页不渲染。
...
- **两种打开方式**：标题栏**左侧**（会话列表按钮右边）的入口主按钮 = **分屏打开**（右侧 iframe 嵌工作台，左侧主视图折叠会话列表，默认五五开、分界可拖双击复位；图标为**外框+中间竖线**——一眼就是"一窗分两栏"）；它 **hover 时右方**弹出的副按钮 = **新标签打开**（整个工作台页面，图标即原空心文件夹 + `<>`）。消息流里的文件产物（文件卡 / 链接 / 原文件弹窗）也走同一入口，自动带上所属会话。
  - **为什么把分屏放主按钮**：主按钮是常驻可见的那个，该承担**最常做的动作**——一边让 Agent 改文件、一边自己核对，正是分屏的存在理由；而"要看整页"是少数情况，交给悬浮翻出的副按钮（IDEA 工具窗也是常驻优先、窗口化次之）。分屏开着时主按钮带 `.icon-btn.active`（全站通用的"已开启"语义，轮盘按钮同款）——✕ 移除后，按钮得自己表达开关态。
  - 关闭分屏**不是标题栏上的 ✕**（已按反馈移除），而是面板内与全局几条路径：工作台活动栏最下方的「关闭分屏」、工作台「更多」菜单的「关闭分屏」、Esc（两侧都能触发）、Ctrl+Shift+E，以及点标题栏的「会话列表」（分屏时它本就被折叠，点它＝把列表拿回来，顺带关分屏）。
