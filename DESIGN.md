...
- **文件展示方式**：设置面板「外观」tab 单行按钮（直接展示 ⇄ 弹窗查看）——read/write/edit/patch 等文件工具卡（含 code 子Agent 同款包装）的内容直显形态切换：**直接展示**（默认，现状）/ **弹窗查看**（收敛为文件链接 chip，点击弹窗查看全文；适配会话相对与 code 项目绝对路径，经 `files/preview` 取数，详见「会话临时文件查看与下载」与「文件展示方式」渲染说明）。`localStorage["gebai.ui.fileDisplay"]` 仅存 `"popup"`，跨标签 storage 事件同步；渲染是结构性的，切换后重载当前会话消息即时生效。纯展示层能力：不持久化到会话记录
...
- 工具定义可声明 `card: { titleParams?, args?, codeField?, codeLang?, file?, fileOutput?, fileInline? }`，经 `session.tool.get` 随 ToolInfo 下发，前端按声明渲染工具调用卡片，**不在前端硬编码工具名**：
  ...
  - `titleParams`：参数名列表，其值直接拼入卡片标题——**单参数仅显示值**（`🛠 read · src/main.ts`，省略 `key=` 前缀），多参数 `key=value`（`·` 连接）；简单工具的关键参数（如 `open` 的 url、`write` 的 path）一目了然（**路径始终可见**——code 子Agent 的项目绝对路径再长也截断入头部而非整体丢弃，edit/read/write/patch 卡片头始终能看到目标文件）
  ...
  - `file`/`fileOutput`/`fileInline`（文件卡声明，「文件展示方式」设置的**弹窗查看**模式据此渲染）：`file` 为路径参数名（如 read/write/edit/patch 的 `path`，code 子Agent 同款包装自动继承）——弹窗查看模式下参数区内容类参数（write 的 content 代码块、edit 的旧/新对比块、patch 的 diff 块）收敛为**文件链接 chip**（图标 + 文件名 + 路径 + 下载，点击弹窗查看文件，其余标量参数仍键值行展示）；`fileOutput=true` 表示 output 即文件内容（如 `read`）——该模式下输出同样收敛为链接（失败/拒绝输出无文件引用，照常展示不吞错误）；`fileInline=true` 表示 `codeField` 参数即文件全文（如 `write` 的 content）——chip 弹窗**内联渲染该参数内容**（审批前文件尚未落盘，取数会看到旧内容，审查对象是待写入内容本身）；read/write 的 file/image 产物块在该模式下与 chip 重复，不再渲染
  ...
  - 已声明示例：全局 `sh`/`py`/`write`（code 模式 + 文件卡 fileInline）、`edit`（edits 模式 + 文件卡）、`read`（文件卡 + fileOutput）、`fetch_url`/`agent_load`/`agent_run`/`ls`/`grep`/`glob`/`file`（标题参数）、`patch`（code 模式 + 标题参数 path + 文件卡）、`show`/`diff`（block 模式）；code `search_symbols`/`analyze`（标题参数）、`env_detect`/`system_info`（无参数区）；widgets `save`/`get`/`delete`（标题参数）；desktop `screenshot`/`window_focus`/`key_press`/`window_list`/`clipboard_read`/`screen_info`；playwright `open`/`new_page`/`serve_dir`/`switch_page`/`close_page`/`press`/`pages`/`close`
...
- **文件预览（`files/preview`，工具卡文件链接取数入口）**：`read`/`write`/`edit`/`patch` 等文件工具（含 code 子Agent 同款包装）成功执行后结果携带**解析后文件引用**（`ToolResult.file` → 工具消息 `Message.file`：`scope=session` 为会话 `tmp/` 逻辑路径、`scope=fs` 为绝对路径——code 项目文件经 project 参数/粘性根/预置项目解析后的真实路径），前端「文件展示方式=弹窗查看」时据此渲染**文件链接 chip**（点击弹窗查看）；取数统一走 `GET /sessions/:id/files/preview?path=`：**相对路径以会话 `tmp/` 为根**（与 content 同规则），**绝对路径按用户隔离边界放行**——沙箱用户仅允许本用户数据目录（`users/{user}/`，与文件工具 project 参数经 `resolveInSandbox(root=users/{user})` 的可达范围一致，含符号链接逃逸检查），非沙箱（本地模式操作者本人，与文件工具能力对齐）放开；`?download=1` 以附件形式返回（文件卡/chip 的下载入口）；归属判定在工具执行时以会话 `tmp/` 真实绝对路径（`sessionPath` 拼接）完成（项目绑定工具的 workdir 是项目根不能作依据），前端不猜测路径解析
...
- **文件展示方式（「弹窗查看/直接展示」，设置面板「外观」，`web/file-display.ts`）**：`read`/`write`/`edit`/`patch` 等文件工具卡（含 code 子Agent 同款包装）自带的内容直显可整体切换——**直接展示**（默认，现状：write 的 content 代码块、edit 的旧/新对比块、read 的文件内容输出与 file/image 产物卡按原样内联渲染）或**弹窗查看**（收敛为**文件链接 chip**：📄 图标 + 文件名 + 路径（弱化单行省略）+ 下载图标，点击弹窗查看文件全文；`write` 类 `fileInline` chip 弹窗内联渲染待写入内容（审批审查参数内容本身，落盘前后不受影响））；chip 路径优先用**结果解析后的真实文件引用**（`Message.file`——`scope=session` 会话 `tmp/` 逻辑路径 / `scope=fs` 项目绝对路径，经 `files/preview` 取数），调用时（结果未达）回退原始参数路径（会话相对路径可直接解析，项目相对路径需等结果升级——服务端完成 project 参数/粘性根/预置项目解析，前端不猜测）；**实时链路结果到达后 chip 自动升级为解析后路径**（`upgradeFileLinks`，内联内容 chip 不升级）；read 的文件内容输出（`fileOutput`）与 read/write 的产物块在弹窗模式下与 chip 重复，不再直显（失败/拒绝输出无文件引用，照常展示不吞错误）；localStorage `gebai.ui.fileDisplay` 仅存 `"popup"`，跨标签 storage 事件同步，切换后重载当前会话消息即时生效（渲染是结构性的）
