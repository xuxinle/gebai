...
- **路径沙箱**：服务端部署模式下，文件类工具（`read`/`write`）仅允许访问当前用户的 `users/{user}/` 数据目录及会话 `tmp/`，对目录外路径一律拒绝，防止路径穿越（`../`、绝对路径、符号链接）；**桌面/本地浏览器模式默认不启用目录限制**，可访问本机任意路径（由 `GEBAI_SANDBOX` 控制，默认按运行形态自动选择）；**admin 用户豁免路径沙箱**（特权用户，`Sandbox.isExempt`/`enforcedFor(user)` 按用户判定：豁免用户即使沙箱启用也按本地模式放开，普通用户受约束；无独立服务身份）
...
- **Webhook SSRF 防护**：Webhook 注册默认拒绝回环/链路本地/云元数据地址（`localhost`、`127.*`、`169.254.*`、`::1`、`fe80:*` 及其 IPv4-mapped/尾点 FQDN 等绕过形式），需内网回调时以 `GEBAI_WEBHOOK_ALLOW_PRIVATE=true` 显式放开
...
export const tools: ToolSet = { read, write, edit, apply_patch, sh, py, ls, grep, search_files, search_symbols, move_file, delete_file, diff, analyze, git, fetch_url, ask_user, agent_run, todo, preview_server, page_capture, vision }
export const requiresApproval = { edit: true, write: true, apply_patch: true, sh: true, py: true }
...
- **工作流**：沿用 code 的「规划→探索→定位→方案→修改→验证→收尾」（探索段含 `search_symbols` 符号定位与 grep 引用搜索分工、修改段 `apply_patch` 补丁应用优先——先 read 确认当前内容、上下文 2~4 行、一次补丁聚焦一个改动点、收尾段 `git` 工具只读核对且不与既有未提交改动混淆）；**设计同步铁律**：任何修改行为/接口/协议/存储布局/常量/命名规则等设计层面变更，必须同步更新 `DESIGN.md` 对应章节（文档与代码保持一致）；修改范围仅限子Agent 目录与允许的配置项，核心引擎源码默认只读（`GEBAI_SELF_MODIFY=true` 显式开启）；**测试是唯一准入凭证**（先跑相关测试文件，失败修复或回滚——失败先看错误信息定位再修，不盲目重试）；收尾前用 `ask_user` 确认验证方式——UI/前端类修改经 `page_capture` 捕获当前页面验证（先请用户刷新页面拿到最新构建），服务端功能类修改经 `preview_server` 启动临时验证服务，验证结束即停止
- **与 `code` 的差异**：
...
- **生效方式**：
  - 脚本调试模式：修改源码后直接生效（子Agent 热加载）
  - 二进制模式：修改后的代码进入下次构建；运行期通过环境变量覆盖提示词/配置（会话级）实现即时调优
- **安全约束**：自我修改走统一审批流；修改范围限定子Agent 目录与允许的配置项，核心引擎源码默认只读（可通过 `GEBAI_SELF_MODIFY` 显式开启）
- **测试门槛**：任何自我修改必须通过相关测试才能落盘，防止退化；测试失败自动回滚本次改动（测试策略与覆盖率门槛见「测试策略」）
...
- **会话过期策略**：本地/服务模式均可配置会话闲置过期时间（默认 90 天，按 `chat.json` 最后修改时间判定），过期会话归档到 `trash/` 后可恢复，超保留期删除
...
- **上下文占用口径**：以模型接口返回的 usage 真值为基准——每次真实调用后记录其 `input_tokens`（含 system 提示词与工具 schema，即「真上下文」）与该次调用已覆盖的消息条数，之后尚未发送的增量消息以 chars/4 估算补足（下一次真实调用会以新真值接管）；跨 run 持久化基线（`SessionData.ctxInputTokens`/`ctxAtMessage`），下次 run 触发判定 = 基线 + `history.slice(ctxAtMessage)` 增量估算；接口不返回 usage（或压缩替换消息导致索引锚点失效）时回退全量估算
- 压缩策略（按序使用）：
...
- **每用户事件日志（seq + 有界环形缓冲，容量 1000）**：服务端级单点订阅事件总线，按会话归属路由到所属用户的日志，分配全局递增 `seq`；在线推送即日志条目的实时投递。**断线期间事件照常入日志**（与连接数无关），重连后客户端按 `seq` 增量重放（`sync.request` → `sync.replay`），离线事件不丢失；缓冲溢出（缺口）返回 `overrun`，客户端走全量重同步（`session.get` 等）收敛
...
- **流式任务断线恢复**：`sendPrompt` 断线时**挂起**（不抛错），等待自动重连后按 `seq` 重放离线事件无缝续流；任务未确认接收时以快照判定（运行中→恢复；已跑完→从存储合成收尾；未开始→补发一次，遇「任务已在运行」视为已接受）；`overrun` 走全量重同步（`resume` 重置 + 存储内容重建）
...
- **身份映射**：本地模式全部映射到 admin 用户；服务模式按飞书用户 `open_id` 自动创建 GEBAI 映射用户（用户名 `feishu_{open_id}`，随机密码不可密码登录，角色普通用户，管理员可在用户管理禁用），10 分钟内存缓存
- **会话映射**：每个飞书单聊/群聊自动关联独立 GEBAI 会话（`feishu_{chat_id}`，会话名取飞书会话名称），消息上下文与 Web UI 完全互通；群聊成员共享同一会话（引擎身份为会话创建者，不因他人发言重建/覆盖）
...
- **交互命令**：`/help`、`/new`（清空当前对话上下文重建会话）、`/sessions`、`/cancel`、`/approve`、`/reject`（批准/拒绝最近一个待审批工具调用，**仅审批发起者可操作**）、`/approval-skip`（服务模式非管理员拒绝）
...
- **安全**：长连接为出站连接（不暴露回调端口）；凭证仅存环境变量（本地 `.env` 不入版本库）；服务模式按飞书用户隔离数据与权限（映射用户为普通角色）；消息/会话 id 白名单校验（`[A-Za-z0-9_-]{1,64}`）防路径注入；卡片按钮回调的 chatId/openId 同样白名单校验 + 发起者授权
