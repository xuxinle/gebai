...
- **内置风格**：内置多套风格，随二进制分发，切换无需重新部署：
  ...
  | `qinhan` | 秦汉彩绘：玄漆黑底（秦尚黑）+ 朱红彩绘（汉尚赤）+ 鎏金勾边（配色取自秦兵马俑彩绘与汉漆器，辅色石青/石绿/赭石/汉紫矿物颜料色），**小圆角方正庄重**（皇家规制）；纹饰——**云气纹底饰**（卷云线描暗纹铺底）+ **标题栏锯齿纹带 + 回纹边带**（漆篋/青铜器边饰语汇，两缘渐隐）+ **顶部鎏金/朱砂双钩线** + **漆器双线面板**（鎏金外线 + 朱砂内线复合描边）+ 消息气泡**四角金角花**（函套角）、宋体碑刻标题/表头/榜书匾额；**空状态朱雀瓦当徽章**（双圈轮界 + 回首凤鸟剪影 + 卷云式三股尾羽，鎏金线描）+ 四角函套金饰 + 菱纹金分隔线；特效**云气祥纹**（卷云线描缓移缓旋，鎏金/朱砂/汉紫三色 + 鎏金微尘上浮） |
