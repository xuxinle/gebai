...
- **两种打开方式**：标题栏**左侧**（会话列表按钮右边）的入口主按钮 = **新标签打开**（工作台是重量级独立页面，与聊天并行是常态，且故障隔离；图标为空心描边文件夹 + `<>`）；它 **hover 时右方**弹出的副按钮 = **分屏模式**（右侧 iframe 嵌工作台，左侧主视图折叠会话列表，默认五五开、分界可拖双击复位）。消息流里的文件产物（文件卡 / 链接 / 原文件弹窗）也走同一入口，自动带上所属会话。关闭分屏**不是标题栏上的 ✕**（已按反馈移除），而是面板内与全局几条路径：工作台「更多」菜单的「关闭分屏」、Esc（两侧都能触发）、Ctrl+Shift+E，以及点标题栏的「会话列表」（分屏时它本就被折叠，点它＝把列表拿回来，顺带关分屏）。入口那组因此永远只表达"打开"。
  ...
  - **面板是纯容器，没有标题栏**：只有分界拖条 + iframe。那条 30px 横条要花掉整个编辑区的垂直空间，而它的按钮各有去处——**关闭＝也不在标题栏**（走面板内与全局路径：工作台「更多」菜单的「关闭分屏」、Esc、Ctrl+Shift+E、点「会话列表」）；打开＝入口主按钮；重新加载／在新标签打开／关闭分屏＝工作台自己的「更多」菜单（页面级动作归页面自己）。入口那组按钮因此保持原状：副按钮永远只是分屏图标、只在浮空时出现，不随状态变形或常驻（分屏开着时它仍是 toggle，提示文案随之变为"关闭分屏"——这也正是它仍挂着 toggle 行为的原因：入口自己不再承担"显式的关闭按钮"语义，但多一条顺手的关闭路径不亏）。
