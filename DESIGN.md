...
| `multi_turn`（**多轮交互**） | 多轮请求-响应往返（非流式），有往返但无前端页面；**仅关键操作（requiresApproval）询问用户**（如飞书审批回调卡片），非关键操作自动 | 飞书机器人 | `none` + `multi_turn` 声明（ask_user 有选择卡片适配；show 图表分支走飞书后端渲染） |
| `realtime`（**实时交互**） | 实时流式交互：完整前端，关键操作询问用户 | WebSocket（Web UI，默认） | 全部 |

- **工具声明**（`interaction: "realtime"` 仅实时前端）：`page_capture`/`ask_env` 与 widgets 四工具（`save`/`list`/`get`/`delete`，依赖前端页面配合，ask_env 需填值弹窗）；`interaction: "multi_turn"`（至少多轮交互）：`ask_user`/`plan`（飞书通道已有选择卡片适配）；其余工具缺省 `none`；**合并型工具不做工具级声明**——`show`（图表/HTML/文件三分支）全模式可见，按 `ctx.interactionMode` 在分支内校验通道能力（html 分支仅 realtime、图表分支 none 下直接引导 `render=backend`），见「内容展示」
...
- **飞书通道** = `interactionMode: "multi_turn"`：realtime 声明的工具（`page_capture`/`ask_env`/widgets 四工具）自动禁用（原 `FEISHU_DISABLED_TOOLS` 名单已移除，由声明统一驱动），`ask_user` 可用（走飞书交互卡片）；`show` 不做工具级禁用（html 分支明确报错引导改用其他方式、图表分支经飞书后端渲染出图），关键操作经审批卡片询问
- **REST 通道** = `interactionMode: "none"`：`ask_user` 及实时前端工具自动禁用，不再等待至超时；`show` 不做工具级禁用（html 分支明确报错、图表分支直接引导 `render=backend`，均不空等超时）；本地模式需审批工具自动通过，**服务模式需审批工具直接拒绝**（防免审批执行）；需要完整交互能力请走 WS 通道
