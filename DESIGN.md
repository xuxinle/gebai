...
export const tools: ToolSet = { read, write, edit, patch, sh, sh_task, py, ls, grep, glob, search_symbols, file, diff, analyze, git, project, fetch_url, web_search, ask, agent_run, todo, preview_server, env_detect, system_info }
...
- **分析工具**：`read`（读源码，默认带行号可直接引用 文件:行号；`encoding` 解码非 UTF-8 文件）、`ls`（目录结构）、`grep`（内容搜索：files/count 形态摸底 + content/context 精读（contextBefore/After 非对称）、include/exclude 过滤、literal 字面匹配代码片段、head_limit 压低上限）、`glob`（文件名 glob 查找，支持 `{a,b}` 花括号与 exclude 排除）、`search_symbols`（tree-sitter 符号搜索双模式：**定义**定位——解析函数/类/方法/类型定义，返回 文件:行号: 类型 名称，精确匹配优先；`mode=references` **引用/调用点**——叶子节点精确匹配 + 排除定义名与注释/字符串（比 grep 文本匹配少误报），梳理调用链/影响面用）、`analyze`（tree-sitter 语法结构概览）、`diff`（修改前后/两文件对比，UI 并排高亮）、`web_search` + `fetch_url`（联网检索第三方库/框架文档：搜索定位 + 抓取详情页）、`sh`（运行测试）
...
- **目录驱动、不可自定义**：前端设置面板从服务端 `GET /api/v1/env/catalog` 拉取环境变量目录（白名单，含变量作用说明），**按「全局 / 各子Agent」分组展示**（全局：模型相关 `GEBAI_LLM_*`/`GEBAI_VISION_*`、联网搜索 `GEBAI_SEARCH_*`、审批跳过、代理/时区等；子Agent 组：`{AGENT_NAME_UPPER}_*` 前缀变量如 `CODE_PROJECTS`/`SELF_OPTIMIZE_PROJECT`/`FEISHU_DOCS_APP_ID`）；**子Agent 组由各子Agent 导出 `envVars` 声明汇总**（`core/env-catalog.ts` 不做子Agent 硬编码，新增子Agent 环境变量只需在子Agent 定义中声明，见「子Agent文件格式」）；**未配置的项显示为空、请求不携带**；鼠标悬停（tip）显示变量作用
...
| `web_search` | **联网搜索**（查第三方库文档/报错信息/技术方案/库选型）：返回 标题+URL+摘要 清单（`query` + 可选 `count` 默认 8 上限 10），抓取具体页面全文用 `fetch_url`；搜索服务三选一经 `GEBAI_SEARCH_PROVIDER`（brave/serper/tavily）+ `GEBAI_SEARCH_API_KEY` 配置（前端设置面板或环境变量，未配置时返回配置引导可配合 `ask` 向用户索取）；三家均为固定公网 API 端点（无用户可控 URL，无 SSRF 面），统一 15s 超时，401/403/429 按状态码给病因提示（不回显含密钥诊断的响应体） | 否 |
...
- **执行**：命令参数由 action 枚举构造（status/diff/log/branch/ls-files 无用户字符串拼接）；`show`/`ls-files` 携带用户字符串（`ref`/`path`）——经元字符黑名单校验（拒绝 `"`/`&`/`|`/`<`/`>`/`^`/`%`/反引号等 cmd/shell 注入形态）后引号包裹拼入，`ls-files` 以 `--` 分隔防 pathspec 被解读为选项；`grep` 的 `pattern` 用**放宽黑名单**（仅拒 `"`/`%`/`$`/反引号——引号内活动元字符；`&`/`|`/`<`/`>` 在双引号内为字面量**放行**，正则交替 `a|b` 语法需要）并以 `-e` 定界模式、`--` 分隔 pathspec；以 `dir` 为 cwd 执行对应 git 命令；非零退出码附 stderr（非仓库目录等可诊断信息）；输出走统一截断保护，`ls-files` 附结构化 `data.files`
...
| web_search 服务与条数 | brave/serper/tavily；默认 8 条、上限 10 | `GEBAI_SEARCH_PROVIDER` + `GEBAI_SEARCH_API_KEY` 配置（未配置返回引导）；三家固定公网 API 端点，统一 15s 超时；`count` 参数可调（`WEB_SEARCH_COUNT_*`） |
