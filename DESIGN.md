...
- **人民币招财猫**：人民币主题（`cny`）专属**悬浮玩偶 + 爆金币特效**（`cny-cat.ts` 玩偶交互 + `coin-fx.ts` 全屏 canvas 粒子引擎），仅 `data-theme=cny` 时挂载（监听 `gebai:theme-change` 同步增删）：
  ...
  - **拖动金币/纸币轨迹**：拖动期间沿指针位置混撒小尺寸金币与纸币（30ms 节流：每拍一枚小金币（半径 4-6.5px）+ 半数概率一张小纸币（宽 30-40px）），**漂浮消散**——负净加速度缓升 + 强空气阻力衰减初速 + 轻摆慢转 + 不与地面交互，升空渐隐（粒子引擎按粒子覆盖重力/地面交互，区别于爆发的坠落反弹物理）；点击与拖动按累计位移阈值（6px）判定
