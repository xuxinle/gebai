...
| 设置页 | UI 风格（外观性能模式）、`/approval-skip`（侧栏底部 ⚡）——设置面板；工具启停、子Agent 装载、Webhook 管理无 UI（设置面板不设对应 tab，经 SDK/API 使用） |
...
> **实现**：登录页与设置面板已落地；环境变量/用户/反馈三页统一收敛为设置抽屉（侧栏底部 ⚙️），管理员身份自动显示用户/反馈 tab。工具启停、子Agent 装载与 Webhook 管理原设置面板 tab 已移除（对使用者无实际意义：启停/装载由模型经 `agent_load` 等自主完成、Webhook 属服务端集成配置），对应能力保留在服务端 API 与 SDK 方法中。
