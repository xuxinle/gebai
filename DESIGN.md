...
- **Git 图形化（任意两端对比为一等公民）**：差异统一抽象为**端点**——`WORKTREE` / `INDEX` / 空串（与 WORKTREE 搭配 = 未暂存）/ 任意 rev（提交/分支/标签/`HEAD~n`/SHA）；`compare(from,to)` 支持提交↔提交、提交↔工作区、提交↔暂存区、暂存区↔工作区、工作区↔历史提交、分支↔分支（`mergeBase` 三点语义）；`contentAt(ref,path)` 供 Monaco DiffEditor 取 **两侧真实文本**（非解析 patch，CRLF/编码差异下表现正确）；前端「比较」标签页两端各有端点选择器（工作副本/引用/本地分支/远程分支/最近提交/手输 rev）+ 交换 + `...` 开关 + path 过滤。工具窗为**底部停靠的三栏**（IDEA tool window 式）：变更内容（分组/逐行暂存/丢弃自动 stash 备份/提交框 sticky 底部）/日志（图、过滤、分页、右键操作、提交详情）/分支——「分支」栏内再切标签、暂存、远程（fetch/pull/push force-with-lease）；破坏性写操作默认先备份并返回可恢复引用；`git blame` 追溯、冲突四方内容（base/ours/theirs/current）查看。
