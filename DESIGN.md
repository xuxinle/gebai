...
- **两种打开方式**：标题栏**左侧**（会话列表按钮右边）的入口主按钮 = **新标签打开**（工作台是重量级独立页面，与聊天并行是常态，且故障隔离；图标为空心描边文件夹 + `<>`）；它 **hover 时右方**弹出的副按钮 = **分屏模式**（右侧 iframe 嵌工作台，左侧主视图折叠会话列表，默认五五开、分界可拖双击复位）。消息流里的文件产物（文件卡 / 链接 / 原文件弹窗）也走同一入口，自动带上所属会话。关闭分屏是标题栏**右侧**的独立 ✕（位置固定，不随入口移动）——入口那组因此永远只表达"打开"。
  ...
  - **面板是纯容器，没有标题栏**：只有分界拖条 + iframe。那条 30px 横条要花掉整个编辑区的垂直空间，而它的按钮各有去处——关闭＝标题栏上的**独立关闭按钮**（分屏激活时才出现；入口那组按钮因此保持原状：副按钮永远只是分屏图标、只在浮空时出现，不随状态变形或常驻）；打开＝入口主按钮；重新加载／在新标签打开＝工作台自己的「更多」菜单（页面级动作归页面自己）。
