...
- **单轮计时器**：任务（单轮对话）运行期间在当前流式助手消息 meta 行显示本轮耗时（`.msg-timer`，`msg-time` 旁，250ms 刷新；流式消息元素惰性创建/封段重建时 span 迁移复用到最新元素），任务结束定格最终耗时。默认开启；设置面板「外观」tab 单行按钮可关（`localStorage["gebai.ui.turnTimer"]` 仅存 `"off"`，根元素 `data-turn-timer="off"` 时 CSS 隐藏全部计时元素，跨标签页 storage 事件同步，运行中被关闭即时停表摘除）。纯展示层能力：不持久化到会话记录（`Message` 无耗时字段），历史回放不显示
