...
- **总Agent 系统提示词**（`buildSystemPrompt`）：「复杂/多步操作优先编排一次执行，避免大量单步工具调用浪费往返与词元：固定流程用 flow（引用映射/分支/循环，语法见其工具描述，编排前可用 tool_schemas 查询输出结构），逻辑复杂（动态参数/条件重试/跨步骤聚合）用 js 脚本动态编程，纯系统操作用 sh/py 脚本」（flow/js 的完整能力语义单源于各自工具描述，系统提示词只做一句选型指引——原文复述工具描述曾双倍占上下文）
