...
src/
  ...
  core/             # 领域核心（根目录只放构建生成物，源码按域入子目录）：
    ...
    tools/          #   全局工具域：注册文件（fs/exec/show/agent/interact/flow/extras）+ shared.ts（GlobalToolEntry 契约）+ index.ts（聚合器/barrel）+ projects.ts/vision.ts
    ...
    exec/           #   脚本执行：js-tool/sh-tasks/flow
