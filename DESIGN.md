...
- **消息互通**：文本消息（群聊自动剥离 `@_user_N` 提及占位）→ `session.prompt`；图片消息 → 下载为附件（魔数探测 mime）进入会话；增量文本节流合并（60 字符/1.5s）为「✍️ 预览」消息，任务完成发最终 **interactive 卡片**（JSON 2.0 markdown 渲染，超长截断并提示 Web UI 查看，**以「回复」引用原用户消息**）；工具调用发「🔧 正在执行」状态消息（同任务至多一条）。**消息保留策略：机器人发出的所有消息一律不撤回**（预览/状态/滚动提示/审批与选择卡片均留痕），仅最终回复引用原用户消息；唯一例外是「Typing」表情反应（非消息本体，输出完成后移除）
